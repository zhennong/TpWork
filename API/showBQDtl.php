...
function submitForm(){	
	$snoopy = new Snoopy;
	$snoopy->fetch("http://appserver.nongyaotong.com/nyt_wx/showBQDtl?certificateNo=".$_GET['certificateNo']); 		//获取所有内容
...
function replace($str){
	...
	$str = str_replace("<title>","<meta name=\"viewport\" content=\"initial-scale=1.0; maximum-scale=1.0; user-scalable=no;\" /><title>",$str);
	$str = str_replace("</head>","<link rel=\"stylesheet\" type=\"text/css\" href=\"css/override.css\" /></head>",$str);
	$str = str_replace("<body>","<body style=\"background:#fff;\"",$str);
